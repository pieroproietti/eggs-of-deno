...
export class FileSystem {
  ...
  constructor(config: IEggsConfig, distro: IDistroInfo) {
    ...
    const workRoot = path.join(Constants.NEST, "chroot");
    this.dirs = {
        merged: workRoot,
  ...
  async bind(options: any) {
    Utils.title("🔗 Binding Live Filesystem (OverlayFS)");
    
    // Header script
    this.bindScript.push("#!/bin/bash", "# Auto-generated by eggs-of-deno", "");
   ...
   */
  async unbind() {
    Utils.title("🔌 Unbinding Filesystem");
  ...
  private async mountOverlay(dirName: string) {
    ...
    // Add unbind commands (LIFO: unmount merged first, then lower)
    this.unbindScript.unshift(`umount ${lower}`);
    this.unbindScript.unshift(`umount ${merged}`);
  }
  ...
  private async mountBindRo(dirName: string) {
    ...
    // Add unbind command
    this.unbindScript.unshift(`umount ${target}`);
  }
  ...
  private async execBind(cmd: string) {
    ...
    // Attenzione: questa è una semplificazione, comandi complessi con quote richiedono un parser
  ...
  private async applyUsrMergeFix() {
    ...
    for (const link of links) {
        ...
        // Se è una directory vera, la rimuoviamo (è un errore di copia/rsync)
  ...
  private async saveScripts() {
    ...
    const bindFile = path.join(binDir, "bind.sh");
    await Deno.writeTextFile(bindFile, this.bindScript.join("\n"));
    await Deno.chmod(bindFile, 0o755);
    console.log(`📝 Script bind generato: ${bindFile}`);

    const unbindFile = path.join(binDir, "unbind.sh");
    await Deno.writeTextFile(unbindFile, this.unbindScript.join("\n"));
    await Deno.chmod(unbindFile, 0o755);
    console.log(`📝 Script unbind generato: ${unbindFile}`);
  }

  private finalizeUnbindScript() {
    // Add header at the beginning
    this.unbindScript.unshift("#!/bin/bash", "# Auto-generated by eggs-of-deno - Unbind", "");

    // Add cleanup at the end
    this.unbindScript.push(
        "", 
        "# Cleanup",
        `rm -rf ${this.dirs.lower}`,
        `rm -rf ${this.dirs.upper}`,
        `rm -rf ${this.dirs.work}`,
        "# Be careful removing merged if it contains other things",
        // `rm -rf ${this.dirs.merged}` // Optional: usually keep the root dir
    );
  }
